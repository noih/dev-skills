...
- **No AI attribution** — never add `Co-Authored-By`, `Generated by`, or any AI-related trailers/footers. Some agent runtimes inject such a trailer by default, so read back the finished message before committing and strip it if present. A rule stated only in this file does not reach a runtime that never loaded this file — if the trailer keeps reappearing, put the rule in the always-loaded instruction file for that runtime, or enforce it with a `commit-msg` hook
